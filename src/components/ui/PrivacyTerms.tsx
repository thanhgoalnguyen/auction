...
export default function PrivacyTerms({showMore}: PrivacyTermsProps)  {
	const linkPrivacyTerms = [
		{
	...
	return (
		<div className="privacy-terms flex flex-col items-center w-full mt-auto">
			<div className="flex flex-col gap-6 w-full max-w-[768px] pb-[13px] px-4">
				<div className="flex flex-col gap-[23px]">
					{showMore &&
						<>
							<button className="flex justify-between items-center pr-[34px] cursor-pointer">
					...
					<div className="group">
						<label className="flex justify-between items-center pb-4 pr-[34px] border-b border-dark-200 cursor-pointer">
						...
						<div className="w-full max-h-0 overflow-hidden transition-[max-height] duration-300 group-has-[input:checked]:max-h-[170px]">
							<div className="flex flex-col gap-[29px] w-full pt-[21px] pb-[25px]">
								{
									linkPrivacyTerms?.map((item, index) => 	
										<Link 
											to={item?.link} 
