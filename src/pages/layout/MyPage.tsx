			...
			<div className="container flex flex-col gap-7 text-[11px] leading-[13px]">
				<div className="flex justify-center items-center gap-9 px-5">
					<div className="flex flex-col items-center gap-2">
						...
						/>
						<p className="text-[15px] leading-[18px] text-neutral-600">
							Y.Sara1985
						</p>
					</div>
					<div className="flex items-center gap-4">
						<div className="flex flex-col gap-2">
							<div className="flex justify-between items-center gap-3 w-full">
								<div className="flex items-center gap-1">
									<img src={star} alt="star" className="w-4 h-[15px]" />
									<p className="text-[12px] leading-[15px]">4.2</p>
								</div>
								<p className="text-[6px] leading-[7px]">/ 38件の評価</p>
								<div className="flex items-center gap-2">
									<img src={blonze} alt="blonze" className="w-[14px] h-4" />
							...
							<div className="flex flex-col gap-1">
								{listProgress?.map((item, index) => (
									<div className="flex items-center gap-1" key={index}>
										<p className="w-[14px] text-[7px] leading-[8px] text-neutral-600 text-left">
											{item?.label}
										</p>
										...
										<p className="text-[5px] leading-[6px] text-neutral-600">
											{item?.value}
										</p>
									</div>
								))}
							...
							<div className="flex justify-between items-center w-full">
								<div className="flex items-center gap-3">
									<p className="text-[10px] leading-[12px] text-neutral-600">
										６
									</p>
									<p className="text-[6px] leading-[7px] text-neutral-700">
										出品数
									</p>
								</div>
								<div className="flex items-center gap-3">
						...
						<Link to="/layout/">
							<img src={arrowRight} alt="arrow" className="w-[5px] h-[8px]" />
						</Link>
					</div>
				</div>
				<div className="flex flex-col gap-9 w-full">
					<MyPageNavigation title="商品管理" list={productManagementList} />
					<MyPageNavigation title="設定" list={settingList} />
					<MyPageNavigation title="規約・ポリシー" list={policyTermsList} />
					<button
						onClick={handleOpen}
						className="flex items-center w-full py-4 pl-[2px] border-b border-t border-neutral-300"
					>
						<p className="text-[11px] leading-[13px] text-neutral-600">
							ログアウト
						</p>
					</button>
				</div>
