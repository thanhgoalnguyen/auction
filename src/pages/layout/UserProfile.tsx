		...
		<div className="user-profile flex flex-col items-center w-full h-max min-h-full">
			...
			<div className="container flex flex-col w-full grow">
				...
				<div className="flex items-end gap-[42px] px-3 mb-9">
					...
					<Link to="/layout/" className="text-[7px] leading-[8px]">
						もっと見る
					</Link>
				</div>
				<div className="flex items-center mb-2 border-b border-neutral-1100">
					...
					>
						出品中アイテム（8）
						{type === "sell" && (
							<span className="absolute top-full left-0 w-full h-[1px] bg-neutral-600"></span>
						)}
					</button>
					<button
				...
				<p className="text-[8px] leading-[10px] mb-3 ml-3">
				...
				<div className="grid grid-cols-4 gap-x-1 gap-y-7 mb-6">
					{listRecentProduct?.slice(0, number)?.map((item, index) => (
						<ProductPrice key={index} data={item} />
					))}
				</div>
				<div className="px-2 mt-auto">
					<ButtonContainer kind="secondary" onClick={handleLoadMore}>
						もっと見る
					</ButtonContainer>
				</div>
			</div>
